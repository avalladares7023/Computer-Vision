...
#define MS_MAX_NUM_CONVERGENCE_STEPS	5										// limits the number of convergence steps to 10
...
// Constructor - class members called by the compiler every time an object of that class is instantiated
Point5D::Point5D(){
	x = -1;
	y = -1;
}

// Destructor - similar to constructor but when the object scope ends
Point5D::~Point5D(){
}
...
// Scale the Lab color to OpenCV range that can be used to transform to RGB
...
// Mean Shift Filtering -----------------------------
	// works just like the MSFiltering function
	int ROWS = Img.rows;
	int COLS = Img.cols;
...
// Simulates Segmentation 
	int RegionNumber = 0;			// Region number
